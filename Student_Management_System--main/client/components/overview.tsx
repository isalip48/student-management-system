...
export default function Overview() {
  ...
  useEffect(() => {
    // Function to fetch semester information based on registration number
    const fetchSemester = async () => {
      try {
        const regNo = localStorage.getItem('regNo');
  
        const response = await fetch(`http://localhost:5030/api/student/count/${regNo}`);
        if (!response.ok) {
        ...
        if (!data.semester) {
          throw new Error('Semester information not found in response');
        }
